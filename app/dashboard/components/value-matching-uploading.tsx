    ...
    const handleOnSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (groundtruthCsv) {
                ...
                if (groundtruthCsvFile?.name) uploadData.append("groundtruth_csv_name", groundtruthCsvFile.name);
                if (groundtruthCsvFile?.size) uploadData.append("groundtruth_csv_size", `${(groundtruthCsvFile.size / 1024).toFixed(2)} KB`);
                uploadData.append("groundtruth_csv_timestamp", new Date().toISOString());
            }
            ...
            runMatchingTask({
                ...
                onResult: () => {
                    ...
                    setIsVisible(false);
                    toastify("success", "Value matching initialized and ground truth applied.");
                },
                onError: (error) => {
    ...
    >
        ...
        <form encType="multipart/form-data" onSubmit={onSubmit}>
            ...
            <Alert severity="info" sx={{ mt: 2 }}>
                The ground truth will be applied by appending missing candidates and accepting the mapped pairs.
            </Alert>
...
const ValueMatchingUploadButton: React.FC<{ onClick: () => void }> = ({ onClick }) => (
    <IconButton
        color="secondary"
        onClick={onClick}
        sx={{
