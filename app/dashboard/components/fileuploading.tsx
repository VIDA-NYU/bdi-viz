...
const FileUploading: React.FC<FileUploadingProps> = ({
  ...
  const { setIsLoadingGlobal, setTaskStateFor } = useContext(
    ...
    <form encType="multipart/form-data" onSubmit={onSubmit}>
      <Box sx={{ mb: 2 }}>
        <Typography variant="h6" sx={{ fontWeight: 700 }}>
          Upload files for schema matching
        </Typography>
        <Typography variant="body2" color="text.secondary">
          Provide your source dataset and optional target references. If you
          omit optional files, defaults to the GDC v3.3.0 schema.
        </Typography>
      </Box>
      ...
      <Stack spacing={2}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
            Upload a .csv file containing your unstructured or raw biomedical
            dataset. This serves as the basis for matching.
          </Typography>
          <Dropzone
            required
            name="source-csv"
        ...
        <Box>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
            If omitted, the system will default to the GDC v3.3.0 schema. Upload
            a .csv file that reflects the reference or gold-standard biomedical
            dataset (e.g., GDC, PDC).
          </Typography>
        ...
        <Box>
          <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>
            C. Target Schema JSON File{" "}
            <Chip size="small" label="Optional" sx={{ ml: 1 }} />
          </Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
            If omitted, the system will use the GDC v3.3.0 schema definition.
            Upload a .json file that defines metadata and attributes for the
            target schema.
          </Typography>
          <Dropzone
            name="target-json"
  ...
  "education_level": {
    ...
    "enum": [
      ...
      <Alert severity="info" sx={{ mt: 2 }}>
        Tip: You can start with just the Source CSV. The system will infer the
        rest using the default GDC schema.
      </Alert>
